...
- Re-lock reviewer packages after merging template changes. Package manifests and prompts are
  ...
  unloadable. Run from the hub root, listing every reviewer directory:
  `cargo run --manifest-path tools/review-kernel/Cargo.toml -p review-config --example lock --
  .review/reviewers <reviewer>... > .review/review.lock`.
